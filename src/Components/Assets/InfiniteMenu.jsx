...
class InfiniteGridMenu {
  ...
  dispose() {
    ...
    // Best-effort GPU cleanup (safer than trying to individually delete all buffers in this file)
    try {
  ...
  #initTexture() {
    ...
    const currentToken = ++this._textureToken;
    ...
    Promise.allSettled(this.items.map((item) => loadImage(item.image))).then((results) => {
      if (this.disposed || this._textureToken !== currentToken) return;
    ...
    if (canvas) {
      try {
        sketch = new InfiniteGridMenu(
          ...
          list,
          handleActiveItem,
          (moving) => isMounted && setIsMoving(moving)
        );
        sketch.start();
      } catch (err) {
        console.warn('InfiniteMenu: WebGL init failed', err);
      }
    }
